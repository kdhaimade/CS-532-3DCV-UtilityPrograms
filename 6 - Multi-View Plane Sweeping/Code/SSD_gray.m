function [depth_map] = SSD_gray(i_ref, p_right, p_left, depth_range, n_plane, k_ref, win_size)
...
for d = 1:length(p_right)
    dr{d} = (i_ref-padarray(p_right{d},[f f])).^2;
    dl{d} = (i_ref-padarray(p_left{d},[f f])).^2;
end
